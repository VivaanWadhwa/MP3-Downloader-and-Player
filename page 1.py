...
def initialise():
    mixer.init()
    conn=mydb.cursor()
    conn.execute("insert into Blobtable (ImageName, varBinaryData) select 'image1', BulkColumn from openrowset (Bulk 'D:\Users\Vivaan\Documents\GitHub\CS-Project\play.png', SINGLE_BLOB) AS varBinaryData;")
    mydb.commit()
def mainscreen():
    page_1 = tk.Tk()
